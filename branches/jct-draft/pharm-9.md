# PHARM-9: Retrieve Medication Dispenses - Medication Prescription and Dispense (MPD) v1.0.0-comment-2
...
This section corresponds to transaction [PHARM-9] of the IHE Technical Framework. Transaction [PHARM-9] is used by the [Dispense Consumer](actors-transactions.md#dispense-consumer) and the [Dispense Responder](actors-transactions.md#dispense-responder) actors. The Retrieve Dispense [PHARM-9] transaction is used to obtain MedicationRequests for further handling - for example gettting a prescription for dispense.
...
| | |
| :--- | :--- |
| [Dispense Consumer](actors-transactions.md#dispense-consumer) | Queries existing Dispenses from the Order Responder |
...
Upon receiving the `MedicationRequest`, the [Dispense Responder](actors-transactions.md#dispense-responder) is expected to respond with the orders if the conditions are met:
...
Upon receiving the response, the [Dispense Consumer](actors-transactions.md#dispense-consumer) can will store, process or initiate handling of the orders as part of the business process.

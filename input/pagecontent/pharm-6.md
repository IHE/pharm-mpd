...
The Submit Medication Order [PHARM-6] transaction passes a `MedicationRequest` from a [Medication Order Placer](actors-transactions.html#order-placer) to a [Medication Order Receiver](actors-transactions.html#order-receiver). The `MedicationRequest` can be:
